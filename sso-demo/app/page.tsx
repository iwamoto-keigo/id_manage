...
const OTHER_APP_URL = "http://localhost:3002";
const OTHER_APP_LABEL = "サンプルアプリ B";
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col justify-center px-8 py-16 animate-fade-up">
      <div className="space-y-2">
        <p className="eyebrow">サンプルアプリ A</p>
        <h1 className="font-display text-5xl leading-[1.05] tracking-[-0.02em]">
          こちらはアプリ <span className="text-sienna">A</span> です
        </h1>
      </div>
      ...
      <p className="max-w-md text-sm leading-relaxed text-muted">
        Keycloakの <code className="font-mono text-ink">demo</code> レルムを使うNext.jsクライアントです。
        アプリBと同じレルムを共有しているため、片方にログインすれば
        もう片方は自動的にログインされます（シングルサインオン）。
      </p>

      <form action={login} className="mt-10">
        <button
          type="submit"
          className="group inline-flex items-center gap-3 border border-ink bg-ink px-8 py-4 text-sm text-paper transition-colors hover:bg-sienna hover:border-sienna"
        >
      ...
      <dl className="mt-16 grid grid-cols-[auto_1fr] gap-x-6 gap-y-3 font-mono text-[11px] text-muted">
        ...
        <dd className="text-ink">sso-demo</dd>
...
function SignedInView({ session }: { session: DemoSession }) {
  ...
  async function logoutFromSSO() {
    ...
    const postLogout = process.env.AUTH_URL ?? "http://localhost:3001";
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col justify-center px-8 py-16 animate-fade-up">
      <div className="flex items-baseline gap-3">
        ...
        <p className="eyebrow text-forest">SSO認証済み · アプリA</p>
      </div>

      <h1 className="mt-4 font-display text-4xl leading-[1.05] tracking-[-0.02em]">
        こんにちは、
        <span className="text-sienna">{name}</span>
        さん。
      </h1>
      ...
      <div className="mt-12 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-6">
        <a
          href={OTHER_APP_URL}
          className="inline-flex items-center gap-2 border border-sienna px-6 py-3 text-sm text-sienna transition-colors hover:bg-sienna hover:text-paper"
        >
      ...
      <p className="mt-10 max-w-md text-[11px] leading-relaxed text-muted">
        「このアプリからログアウト」は、このアプリ A のセッションCookieだけを消します。
        Keycloak側のSSOセッションは残るため、アプリ B や再度この画面を開いた時は
        パスワード入力なしで再ログインします。
